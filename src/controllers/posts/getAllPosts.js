...
const getAllPosts = async (req, res, next) => {
  try {
    ...
    const getPostDetails =
      "select u.user_id, u.email, u.name, p.post_id, p.title, p.description, p.created_on, p.updated_on, json_agg(json_build_object('link_id', l.link_id, 'link_url', l.link_url)) as link_urls from users u inner join posts p on u.user_id = p.user_id inner join links l on p.post_id = l.post_id group by p.post_id, u.user_id, u.name, u.email limit $1 offset $2";
    ...
    return utility.successResponse(
      res,
      resData,
